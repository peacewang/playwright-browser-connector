...
- 复用 Chrome 浏览器的登录会话、Cookie 和本地存储，无需在网站上重新登录
...
> v1.4.0 起，插件内置 SessionStart hook，自动修复 token 配置，`/reload-plugins` 不会破坏连接。
...
| 命令                       | 说明                                         |
| ------------------------ | ------------------------------------------ |
| `/setup-browser <token>` | 配置 Playwright MCP Bridge 连接，自动写入 3 处配置 |
| `/browser-connector`     | 激活浏览器连接技能，引导使用已有 Chrome 浏览器      |
